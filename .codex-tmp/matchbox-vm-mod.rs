...
impl VM {
    pub fn interpret_no_timeslice(&mut self, mut chunk: Chunk) -> Result<BxValue> {
        chunk.ensure_caches();
        ...
        self.run_until_future_settled_no_timeslice(future)
    }
    ...
    pub fn begin_call_function_value(&mut self, func: BxValue, args: Vec<BxValue>) -> Result<BxValue> {
        if let Some(id) = func.as_gc_id() {
            match self.heap.get(id) {
    ...
    pub fn pump_once_no_timeslice(&mut self) -> Result<()> {
    ...
    pub fn future_snapshot(&self, future: BxValue) -> Result<(u8, Option<BxValue>)> {
        let id = future.as_gc_id().ok_or_else(|| anyhow::anyhow!("Value is not a future"))?;
        match self.heap.get(id) {
            GcObject::Future(f) => match &f.status {
                FutureStatus::Pending => Ok((0, None)),
                FutureStatus::Completed => Ok((1, Some(f.value))),
                FutureStatus::Failed(error) => Ok((2, Some(*error))),
            },
            _ => anyhow::bail!("Value is not a future"),
        }
    }

    pub fn run_until_future_settled_no_timeslice(&mut self, future: BxValue) -> Result<BxValue> {
        loop {
            self.pump_once_no_timeslice()?;
            let (state, value) = self.future_snapshot(future)?;
            match state {
                0 => continue,
                1 => return Ok(value.unwrap_or(BxValue::new_null())),
                2 => {
                    let err = value.unwrap_or(BxValue::new_null());
                    let message = self.to_string(err);
                    anyhow::bail!("{}", message);
                }
                _ => anyhow::bail!("Unknown future state"),
            }
        }
    }
